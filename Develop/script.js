...
$(function () {
  // adds a listener to each button.  The listener will save all the entries simultaneously.
...
saveButtons.each(function(index){
  ...
  $(this).on('click', function(event){
    ...
    saveButtons.each(function(ind){
      ...
      saveText[parseInt(time,10) - 9] = $(saveButtons[ind]).siblings('textarea').val();
    });
  ...
  //check hour block to current hour to determine css styling, if its past present or future
...
// Sets the values of the text areas of a local save file exists
